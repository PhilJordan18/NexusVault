    ...
    <div class="card rounded-3xl w-full max-w-md p-8">
        ...
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                ...
                <p class="text-sm text-[var(--text-secondary)]">{{ __('The recipient will receive an encrypted copy.') }}</p>
            </div>
        </div>
        ...
        <form action="{{ route('shares.store') }}" method="POST" id="share-form">
            ...
            <div class="mb-6">
                <label class="block text-sm text-[var(--text-secondary)] mb-2">{{ __('Recipient email') }}</label>
            ...
            <div class="flex gap-4">
                <button type="button" onclick="hideShareModal()"
                        class="flex-1 py-4 text-[var(--text-secondary)] font-medium hover:text-[var(--text-primary)] transition">{{ __('Cancel') }}</button>
                <button type="submit"
