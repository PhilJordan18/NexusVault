...
final readonly class PasswordService
{
    ...
    public function calculateEntropy(string $password): array
    {
        ...
        if ($length === 0) {
            return ['entropy' => 0, 'strength' => 'very_weak', 'label' => __('Very weak')];
        }
    ...
    public function generate(int $length, bool $upper, bool $lower, bool $numbers, bool $symbols): string
    {
        $chars = '';
        if ($lower) {
    ...
    {
        return match (true) {
            $entropy < 40 => __('Very weak'),
            $entropy < 60 => __('Weak'),
            $entropy < 80 => __('Medium'),
            $entropy < 100 => __('Strong'),
